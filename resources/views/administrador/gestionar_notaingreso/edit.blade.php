                ...
                <div class="col-sm-10 col-md-8 col-lg-6 mx-auto d-table h-100">
                    <div class="d-table-cell align-middle">
                        <div class="text-center mt-4">
                            <h1 class="h2">Modificar Nota De Ingreso</h1>
                            <p class="lead">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="m-sm-4">
                                    <!--route de form -->
                                    <form action="{{ route('notaIngreso.update', $notaIng->id) }}" method="POST"
                                            ...
                                            <select name="idproveedor" class="form-control">
                                                @foreach ($proveedores as $proveedor)
                                                    <option value="{{ $proveedor->id }}"
                                                        @if ($proveedor->id == $notaIng->idproveedor) selected @endif>
                                                        {{ $proveedor->nombre }}
                                                    </option>
                                                @endforeach
                                        ...
                                        <div class="mb-3">
                                            <label class="form-label">Tranajadores</label>
                                            <select name="idempleado" class="form-control">
                                                @foreach ($empleados as $empleado)
                                                    <option value="{{ $empleado->id }}"
                                                        @if ($empleado->id == $notaIng->idempleado) selected @endif>
                                                        {{ $empleado->name }}
                                                    </option>
                                                @endforeach
